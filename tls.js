...
encoding_header = [
  ...
  'gzip, deflate, br',         // Gzip, deflate, and Brotli
  'compress, gzip',            // Compress with gzip
  'deflate, gzip',             // Deflate with gzip
  'gzip, identity',            // Gzip with identity encoding
  'gzip, deflate',             // Common encoding formats
  'br',                        // Brotli compression (modern)
  'br;q=1.0, gzip;q=0.8, *;q=0.1', // Brotli preferred, gzip secondary
  'gzip;q=1.0, identity;q=0.5, *;q=0', // Gzip preferred, fallback to identity
  'gzip, deflate, br;q=1.0, identity;q=0.5, *;q=0.25', // Order with Brotli preferred
  'compress;q=0.5, gzip;q=1.0', // Compress less preferred, gzip primary
  'identity',                  // No compression
  'gzip, compress',            // Gzip and compress encodings
  'compress, deflate',         // Compress and deflate encodings
  'compress',                  // Just compress
  'gzip, deflate, br',         // Common modern encodings
  'deflate',                   // Deflate encoding only
  'gzip, deflate, lzma, sdch', // Gzip, deflate, lzma, and SDCH (used in some browsers)
  'br;q=1.0, deflate;q=0.9, gzip;q=0.8', // Brotli preferred, deflate secondary, gzip tertiary
  'gzip;q=1.0, compress;q=0.5, br;q=0.3', // Gzip preferred, compress less preferred
  'deflate, br;q=1.0, identity;q=0.5', // Deflate preferred, Brotli secondary
  'identity, gzip, deflate, br', // Identity fallback with modern encodings
  'gzip, deflate, sdch',       // Deflate and SDCH compression
  'gzip, bzip2, deflate',      // Adds bzip2 as another encoding option
  'deflate, gzip, x-compress', // Some older formats like x-compress
  'lzma, br',                  // LZMA (used by some systems) and Brotli
];


controle_header = [
  'no-cache',                 // Do not cache the response
  'no-store',                 // No caching or storing of response
  'no-transform',             // No transformations on the response
  'only-if-cached',           // Serve only if cached
  'max-age=0',                // Cache response is immediately stale
  'must-revalidate',          // Requires revalidation for stale data
  'public',                   // Publicly cacheable
  'private',                  // Private, only cached for individual client
  'proxy-revalidate',         // Proxy should revalidate stale data
  's-maxage=86400',           // Shared max-age of one day
  'stale-if-error=86400',     // Use stale data if there’s an error for one day
  'max-age=31536000',         // Cache for one year
  'stale-while-revalidate=3600', // Serve stale data while revalidating for an hour
  'immutable',                // Cache response indefinitely
  'cache, no-cache',          // Cache but validate each time
  'max-age=600',              // Cache for 10 minutes
  'no-store, max-age=0',      // No storing or caching, but requires revalidation
  'must-revalidate, private', // Only validate for the individual client
  's-maxage=0, stale-while-revalidate=120', // No shared cache, serve stale data for two minutes
  'max-age=0, must-revalidate', // Stale data must be revalidated
  'public, max-age=86400',    // Publicly cached for one day
  'private, max-age=0, must-revalidate', // Private cache that must revalidate stale data
  'private, no-cache',        // Private cache but with no caching policy
  'public, s-maxage=3600, stale-if-error=3600', // Public cache for an hour with stale fallback on error
  'max-age=604800, immutable', // Cache for a week, no revalidation
  'no-cache, no-store, must-revalidate', // Aggressive no caching
  'private, no-store',        // Private with no storing of data
  'cache, no-store',          // Cache while disallowing storage
];
...
const queryStrings = [
  "?",  // Standard query starter
  "&",  // Append additional queries
  "=",  // Value assignment in query
